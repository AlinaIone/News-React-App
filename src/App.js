...
function App() {
  // Get the initialState from localStorage.
      ...
      <AlertContext.Provider value={{ stateAlert, dispatchAlert }}>
        <FavoritesContext.Provider value={{ stateFavorites, dispatchFavorites }}>
          <Routes>
            ...
            {/* the * means go throught the end of the url but I get this error: ->  Route path "/news/:id*" will be treated as if it were "/news/:id/*" because the `*` character must always follow a `/` in the pattern. <- */}
